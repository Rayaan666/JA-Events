...
export default function Manifesto() {
  ...
  return (
    <section id="manifesto" className="relative bg-[#F8F5FF] overflow-hidden flex flex-col min-h-screen px-4 sm:px-8 lg:px-20 xl:px-28 pt-16 sm:pt-20 pb-12">
      ...
      {/* Ghost quote mark */}
      <div
        className="absolute right-4 sm:right-8 lg:right-24 top-8 text-[clamp(140px,28vw,400px)] font-serif text-ja-purple/[0.04] leading-none pointer-events-none select-none"
        aria-hidden
      >
        "
      </div>
      ...
      {/* ── Label ── */}
      <motion.div
        className="flex items-center gap-4 mb-auto relative z-10 pt-4"
        initial={{ opacity: 0 }}
      ...
      >
        <h2
          ...
          style={{ fontSize: 'clamp(32px, 7.5vw, 118px)' }}
        >
        ...
        <p className="mt-6 sm:mt-8 text-ja-charcoal/50 text-base lg:text-lg max-w-sm leading-relaxed">
          Every JA experience is shaped around the people taking part — to think, participate, discover, connect and celebrate.
        </p>
      </motion.div>

      {/* ── Five Principles ── */}
      <div className="relative z-10 mt-auto pt-8 border-t border-ja-purple/[0.12]">
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-6 sm:gap-0 sm:divide-x sm:divide-ja-purple/[0.1]">
          {principles.map((p, i) => (
            <motion.div
              key={p.id}
              className="group sm:px-5 first:pl-0 last:pr-0 cursor-default"
            ...
            >
              <div className="text-[10px] font-serif italic text-ja-purple/40 mb-1 sm:mb-2 transition-colors duration-300 group-hover:text-ja-purple/80">
                {p.id}
              </div>
              <h3
                className="font-bold uppercase tracking-widest text-ja-charcoal mb-1 sm:mb-2 transition-colors duration-300 group-hover:text-ja-purple"
